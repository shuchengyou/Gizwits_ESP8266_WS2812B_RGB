...
int8_t ICACHE_FLASH_ATTR gizwitsEventProcess(eventInfo_t *info, uint8_t *data, uint32_t len)
{
    ...
    for(i = 0; i < info->num; i++)
    {
        ...
        {
        case EVENT_RGB_PWR :
            currentDataPoint.valueRGB_PWR = dataPointPtr->valueRGB_PWR;
            GIZWITS_LOG("Evt: EVENT_RGB_PWR %d \n", currentDataPoint.valueRGB_PWR);
            if(0x01 == currentDataPoint.valueRGB_PWR)
            {
                //user handle
            }
            else
        ...
        case EVENT_RGB_Color:
            currentDataPoint.valueRGB_Color = dataPointPtr->valueRGB_Color;
            GIZWITS_LOG("Evt: EVENT_RGB_Color %d\n", currentDataPoint.valueRGB_Color);
            switch(currentDataPoint.valueRGB_Color)
            {
            case RGB_Color_VALUE0:
                //user handle
                break;
            default:
                break;
