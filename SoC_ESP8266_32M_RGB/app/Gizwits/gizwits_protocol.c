...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagRGB_PWR)
    {
        info->event[info->num] = EVENT_RGB_PWR;
        info->num++;
        dataPoints->valueRGB_PWR = gizStandardDecompressionValue(RGB_PWR_BYTEOFFSET,RGB_PWR_BITOFFSET,RGB_PWR_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagRGB_Color)
    {
        info->event[info->num] = EVENT_RGB_Color;
        info->num++;
        dataPoints->valueRGB_Color = gizStandardDecompressionValue(RGB_Color_BYTEOFFSET,RGB_Color_BITOFFSET,RGB_Color_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueRGB_PWR != cur->valueRGB_PWR)
    {
        GIZWITS_LOG("valueRGB_PWR Changed\n");
        ret = 1;
    }
    if(last->valueRGB_Color != cur->valueRGB_Color)
    {
        GIZWITS_LOG("valueRGB_Color Changed\n");
        ret = 1;
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizStandardCompressValue(RGB_PWR_BYTEOFFSET,RGB_PWR_BITOFFSET,RGB_PWR_LEN,(uint8_t *)devStatusPtr,dataPoints->valueRGB_PWR);
    if(dataPoints->valueRGB_Color >= RGB_Color_VALUE_MAX)
    {
        GIZWITS_LOG("[ERROR] valueRGB_Color Error , Illegal Overstep\n");
        return -1;
    }
    else
    {
        gizStandardCompressValue(RGB_Color_BYTEOFFSET,RGB_Color_BITOFFSET,RGB_Color_LEN,(uint8_t *)devStatusPtr,dataPoints->valueRGB_Color);  
    }
